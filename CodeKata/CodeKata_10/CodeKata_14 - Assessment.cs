 ...
 * Input
    ...
    * Exit
    ...
    SMART HOME
    Design the workings of a smart home controller that can serve a clock, an organiser and a 
    lighting controller. 
    ...
    •	CLOCK: Enables the clock functionality of the controller. 
        Notifies the user that the clock is currently active and all subsequent 
        commands would be treated as commands for the clock. 

    •	ORGANISER: Enables the list organiser functionality of the controller. 
        Notifies the user that the organiser is currently active and all subsequent 
        commands would be treated as commands for the organiser.

    •	LIGHTS: Enables the lighting controls of the controller. 
        Notifies the user that the lighting controller is currently active and all 
        subsequent commands would be treated as commands for the lighting controller.

    •	RESET: Resets all functionality of the controller. 
        This will include but not exclusive to resetting Clock’s stopwatch timer list, 
        Users lists in the organiser and resetting the Lights auto on and auto off to their default values. 


    CLOCK : 
    The Clock is capable of displaying the system date, time and running a Stopwatch. 
    ...
    LIGHTS: 
    The light controller stores its own AUTO_OFF and AUTO_ON times for the lights in the house.
    The default AUTO_ON is 18:00 and default AUTO_OFF is 00:00.These parameters are user configurable. 
    The light controller should also be able to return the ON or OFF status of the lights when queried 
    by the user based on the AUTO_OFF, AUTO_ON and system date. It accepts the following commands : 
