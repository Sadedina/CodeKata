...
public class PD13_Tests
{
    ...
    public void FindMatchingParenthesis_WhenValidSentenceAndInputGiven_ReturnsCorrectOutput(
        ...
        int LeftIndex)
    {
        var result = PD13.FindMatchingParenthesis(sentence, RightIndex);

        result.Should().Be(LeftIndex);
    }

    [TestCaseSource(nameof(ExceptionTestScript))]
    public void FindMatchingParenthesis_WhenInvalidSentenceAndInputGiven_ThrowsException(
        string sentence,
        int index,
        string exceptionMessage)
    {
        Action act = () => PD13.FindMatchingParenthesis(sentence, index);
